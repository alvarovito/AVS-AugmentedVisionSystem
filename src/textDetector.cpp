...
{
    ros::init(argc, argv, "textDetector_nodo");
    ros::NodeHandle nodo;
    ROS_INFO("Nodo textDetector creado y registrado");
    ...
    namedWindow("TextDetector", WINDOW_AUTOSIZE);
    namedWindow("TextDetector2", WINDOW_AUTOSIZE);
    ...
    while(ros::ok())
    {
        if(!leftFrame.empty())
        {
            cvtColor(leftFrame, src_gray, CV_BGR2GRAY);
            ...
            SWTimage = SWTransform(&detected_edges);

            components = legallyConnectedComponents(&SWTimage);

            components_image = cv::Mat(leftFrame.rows, leftFrame.cols, CV_8UC3, cv::Scalar(0,0,0));
        ...
        if(!rightFrame.empty())
        {
            cvtColor(rightFrame, src_gray, CV_BGR2GRAY);
            // Reduce noise with a 3x3 gaussian kernel
            blur(src_gray, filtered, Size(3, 3));
            /// Canny detector
            Mat detected_edges;
